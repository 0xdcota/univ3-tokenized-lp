...
contract UniswapV3TokenizedLpFactory is IUniswapV3TokenizedLpFactory, ReentrancyGuard, Ownable {
    ...
    function createUniswapV3TokenizedLp(address tokenA, bool allowTokenA, address tokenB, bool allowTokenB, uint24 fee)
        ...
        override
        nonReentrant
        returns (address uniswapV3TokenizedLp)
    {
        if (tokenA == tokenB) {
        ...
        if (getUniswapV3TokenizedLp[genKey(msg.sender, token0, token1, fee, allowToken0, allowToken1)] != NULL_ADDRESS)
        {
        ...
        uniswapV3TokenizedLp = UniV3LpDeployer.createUniswapV3TokenizedLp(
            pool, token0, allowToken0, token1, allowToken1, fee, tickSpacing, DEFAULT_TWAP_PERIOD
        );

        Ownable(uniswapV3TokenizedLp).transferOwnership(owner());

        getUniswapV3TokenizedLp[genKey(msg.sender, token0, token1, fee, allowToken0, allowToken1)] =
            uniswapV3TokenizedLp;
        getUniswapV3TokenizedLp[genKey(msg.sender, token1, token0, fee, allowToken1, allowToken0)] =
        ...
        emit UniswapV3TokenizedLpCreated(
            msg.sender, uniswapV3TokenizedLp, token0, allowToken0, token1, allowToken1, fee, allVaults.length
        );
    }
